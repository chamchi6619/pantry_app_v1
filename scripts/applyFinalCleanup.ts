...
async function applyFinalCleanup() {
  console.log('🚀 Applying final ROCK SOLID cleanup...\n');

  const plan = JSON.parse(fs.readFileSync('claude-final-cleanup-plan.json', 'utf-8'));
  ...
  // 3. Apply deletes
  console.log('🗑️  Deleting orphaned items...');
...
applyFinalCleanup().catch(console.error);
