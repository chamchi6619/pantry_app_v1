...
class ReceiptService {
  ...
  async processReceipt(
    ...
    try {
      ...
      // Hybrid parser: Universal parser with store-specific hints
      console.log('Using parser: parse-receipt-hybrid');

      // Call Edge Function (it calculates hash internally)
      const response = await supabase.functions.invoke('parse-receipt-hybrid', {
        body: {
          ocr_text: ocrText,
