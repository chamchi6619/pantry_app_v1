/**
 * Recipe Discovery Screen - Production Version
 *
 * Based on "Smart Sections" layout (formerly v4)
 * Organized discovery with horizontal scrolling sections
 * Smart categorization: High Match, Quick & Easy, Use Soon
 *
 * Features:
 * - Personalized recipe recommendations
 * - Pantry-based matching
 * - Filter pills (All, High Match, Quick, Healthy)
...
import { getAllCategoriesWithRecipes, type RecipeDatabaseItem } from '../../../services/recipeDatabaseService';
...
export const ExploreRecipesScreenSupabase: React.FC = () => {
  ...
  const loadRecipes = async () => {
    ...
    setLoading(true);
    try {
      console.log('[RecipeScreen] Loading recipe database with pantry matching...');
      ...
      Object.values(recipesByCategory).forEach(categoryRecipes => {
        categoryRecipes.forEach((recipe: RecipeDatabaseItem) => {
          allRecipes.push({
            id: recipe.id,
            title: recipe.title,
            imageUrl: recipe.image_url || undefined,
            cookTime: recipe.total_time_minutes || recipe.cook_time_minutes || undefined,
            servings: recipe.servings || undefined,
            matchPercentage: recipe.pantry_match_percent || 0,
            matchedCount: undefined, // Not available from recipe_database
            totalIngredients: undefined, // Will be calculated
            cookCard: recipe, // Store full recipe data
            isPersonalized: false,
            source_url: recipe.source_url || undefined,
            priorityReasons: recipe.pantry_match_percent && recipe.pantry_match_percent > 70
              ? ['high_match']
              : [],
          });
        });
      ...
      console.log(`[RecipeScreen] Loaded ${allRecipes.length} recipes from recipe_database`);
