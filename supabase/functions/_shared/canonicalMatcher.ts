...
import { cleanIngredientName } from './ingredientCleaner.ts';
...
/**
 * Log OOV (out-of-vocabulary) ingredient for weekly review
 */
async function logOOV(ingredientName: string, supabaseClient: any): Promise<void> {
  try {
    await supabaseClient
      .from('ingredient_oov')
      .insert({ ingredient: ingredientName });
  } catch (error) {
    // Silent fail - don't break matching if logging fails
    console.error('Failed to log OOV ingredient:', error);
  }
}

/**
 * Find best matching canonical item for an ingredient name
 * @param ingredientName - Raw ingredient name to match
 * @param canonicalItems - List of canonical items to match against
 * @param supabaseClient - Optional Supabase client for OOV logging
 */
export function findMatch(
  ...
  // No match found - log to OOV table for weekly review
  if (supabaseClient) {
    // Fire and forget - don't await to avoid slowing down matching
    logOOV(normalized, supabaseClient).catch(() => {});
  }
...
/**
 * Clean ingredient names in a cook card
 * Removes quantities, units, prep words from ingredient_name field
 * Should be called AFTER extraction but BEFORE saving to database
 */
export function cleanIngredientNames(ingredients: any[]): any[] {
  return ingredients.map(ing => ({
    ...ing,
    name: cleanIngredientName(ing.name || ing.ingredient_name)
  }));
}
